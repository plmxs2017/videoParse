...
from m3u8download_hecoter import parser,download,merge,delFile,idm3
...
def m3u8download(m3u8url, title='',base_uri='',threads=20, key=None, iv=None, method=None, work_dir='./Downloads', headers=None,enable_del=True,merge_mode=3):
    ...
    if m3u8url.split('?')[-2][-4:] == '.mp4':
        idm3.download(url=m3u8url, file_name=title if '.mp4' in title else title + '.mp4')
    else:
