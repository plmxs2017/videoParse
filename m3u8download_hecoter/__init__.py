...
def m3u8download(m3u8url, title='',base_uri_parse='',threads=16, key=None, iv=None, method=None, work_dir='./Downloads', headers=None,enable_del=True,merge_mode=3):
    # 构造m3u8下载信息
    ...
    if '.mp4' in m3u8url and 'm3u8' not in m3u8url:
        ...
        if type(m3u8url) == list:
            for info in m3u8url:
                print(info)
                m3u8download(m3u8url=info['m3u8url'], title=info['title'],base_uri_parse=info['base_uri_parse'])
            sys.exit(0)
        ...
        title, durations, count, temp_dir, data, method = parser.Parser(m3u8url, title,base_uri_parse,method=method, key=key, iv=iv,
